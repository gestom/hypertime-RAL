...
function create_graph
{
	...
	do	
		...
		do
			...
			if [ $(paste $m.txt $n.txt|tr \\t ' '|cut -f 1,3 -d ' '|./t-test|grep -c higher) == 1 ] 	#change fields 1,3 to 2,4 for absolute testing
			then
...
cat best.txt|sort -k 6 >besta.txt
mv besta.txt best.txt
...
for i in $(cut -f 2 -d ' ' best.txt);
do
	model=$(grep $i best.txt |cut -f 2,4 -d ' '|tr ' ' _|sed s/$//|sed s/^/..\\/results\\/$d\\//)
	cat $model.txt|awk '{a=a+$1;b=b+$2}END{print a/NR; print b/NR}' >$model.avg 
	echo \'$(grep $i best.txt |cut -f 2,4 -d ' '|tr ' ' _|sed s/$/.avg/|sed s/^/..\\/results\\/$d\\//)\' 'using ($0+XXX):($1*100) with boxes title' \'$i\',\\|sed s/XXX/\($f-$n*0.5+1\)\\/$n\.0/ >>draw_summary.gnu;   #modify for another error measure
	f=$(($f+1))
done
