...
int newSort(const void* i,const void* j) 
{
...
void CNew::init(int iMaxPeriod,int elements,int numActivities)
{
	maxPeriod = 7*86400;
	numElements = 7*24;
...
int CNew::add(uint32_t time,float state)
{
	if (measurements == 0)
	{
...
void CNew::update(int modelOrder,unsigned int* times,float* signal,int length)
{
	...
	qsort(storedFrelements,numElements,sizeof(SFrelement),newSort);
...
float CNew::estimate(uint32_t time)
{
...
float CNew::predict(uint32_t time)
{
	float saturation = 0.01;
	float estimate =  predictGain;
	for (int i = 0;i<order;i++) estimate+=2*predictFrelements[i].amplitude*cos(time/predictFrelements[i].period*2*M_PI-predictFrelements[i].phase);
...
int CNew::save(char* name,bool lossy)
{
...
int CNew::importFromArray(double* array,int len)
{
	int pos = 0;
...
int CNew::exportToArray(double* array,int maxLen)
{
	int pos = 0;
	array[pos++] = type;
	array[pos++] = order;
...
int CNew::save(FILE* file,bool lossy)
{
	int frk = numElements;
