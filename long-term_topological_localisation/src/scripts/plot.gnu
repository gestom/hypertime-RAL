...
set title "Localization results" 
set xlabel "Number of image features selected [-]"
set ylabel "Localization error rate [%]"
plot [0:49][0:99] \
sprintf("%s/0.txt",map)  using 1:(100-$2*100) with linespoints title "Static model",\
sprintf("%s/1.txt",map)  using 1:(100-$2*100) with linespoints title "FreMEn - order 1",\
sprintf("%s/2.txt",map)  using 1:(100-$2*100) with linespoints title "FreMEn - order 2",\
